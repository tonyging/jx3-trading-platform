...
.main-content.with-sidebar {
  margin-left: 210px; /* 與展開狀態下側邊欄寬度相同 */
  width: 100%;
}

/* 當側邊欄收起時的內容區域樣式 */
.main-content.with-sidebar.sidebar-collapsed {
  margin-left: 72px; /* 與收起狀態下側邊欄寬度相同 */
  width: 100%;
}
...
@media (max-width: 768px) {
  /* 保持側邊欄寬度，但應用側邊欄的收起狀態樣式 */
  .main-content.with-sidebar {
    margin-left: 72px; /* 小屏幕上預設使用收起狀態的寬度 */
  }
}
...
@media (max-width: 480px) {
  .main-content.with-sidebar,
  .main-content.with-sidebar.sidebar-collapsed {
    margin-left: 72px; /* 極小屏幕也保持側邊欄收起狀態的寬度 */
  }
}
</style>
